...
entropy는 데이터가 서로 다른 클래스에 속하면 높고, 같은 클래스에 속하면 낮다. 다시 말하면 각각의 데이터가 특정 클래스에 속할 확률이 높고 나머지 클래스에 속할 확률이 낮다면 entropy가 낮고, 모든 각각의 클래스에 속할 확률이 비슷하다면 entropy는 높다.
...
- `출장지에 비가 내리는데(p)` 모든 친구가 비가 내린다라고 한다면 `모든 친구가 진실을 말하는 것(8/27)`이다. 그 확률은 `8p/27`이다.
- `출장지가 비가 내리지 않는데(1-p)` 모든 친구가 비가 내린다라고 한다면 `모든 친구가 거짓을 말하는 것(1/27)`이다. 그 확률은 `(1-p)/27이`다.
