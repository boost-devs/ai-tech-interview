...
- [TCP/IP의 각 계층을 설명해주세요.](#id-1)
- [OSI 7계층와 TCP/IP 계층의 차이를 설명해주세요.](#id-2)
- [Frame, Packet, Segment, Datagram을 비교해주세요.](#id-3)
- [TCP와 UDP의 차이를 설명해주세요.](#id-4)
- [TCP와 UDP의 헤더를 비교해주세요.](#id-5)
- [TCP의 3-way-handshake와 4-way-handshake를 비교 설명해주세요.](#id-6)
- [TCP의 연결 설정 과정(3단계)과 연결 종료 과정(4단계)이 단계가 차이나는 이유가 무엇인가요?](#id-7)
- [만약 Server에서 FIN 플래그를 전송하기 전에 전송한 패킷이 Routing 지연이나 패킷 유실로 인한 재전송 등으로 인해 FIN 패킷보다 늦게 도착하는 상황이 발생하면 어떻게 될까요?](#id-8)
- [초기 Sequence Number인 ISN을 0부터 시작하지 않고 난수를 생성해서 설정하는 이유가 무엇인가요?](#id-9)
- [HTTP와 HTTPS에 대해서 설명하고 차이점에 대해 설명해주세요.](#id-10)
- [HTTP 요청/응답 헤더의 구조를 설명해주세요.](#id-11)
- [HTTP와 HTTPS 동작 과정을 비교해주세요.](#id-12)
- [CORS가 무엇인가요?](#id-13)
- [HTTP GET과 POST 메서드를 비교/설명해주세요.](#id-14)
- [쿠키(Cookie)와 세션(Session)을 설명해주세요.](#id-15)
- [DNS가 무엇인가요?](#id-16)
- [REST와 RESTful의 개념을 설명하고 차이를 말해주세요.](#id-17)
- [소켓(Socket)이 무엇인가요? 자신 있는 언어로 간단히 소켓 생성 예시를 보여주세요.](#id-18)
- [Socket.io와 WebSocket의 차이를 설명해주세요.](#id-19)
- [IPv4와 IPv6 차이를 설명해주세요.](#id-20)
- [MAC Address가 무엇인가요?](#id-21)
- [라우터와 스위치, 허브의 차이를 설명해주세요.](#id-22)
- [SMTP가 무엇인가요?](#id-23)
- [노트북으로 `www.google.com`에 접속을 했습니다. 요청을 보내고 받기까지의 과정을 자세히 설명해주세요.](#id-24)
- [여러 네트워크 topology에 대해 간단히 소개해주세요.](#id-25)
- [subnet mask에 대해서 설명해주세요.](#id-26)
- [data encapsulation이 무엇인가요?](#id-27)
- [DHCP를 설명해주세요.](#id-28)
- [routing protocol을 몇 가지 설명해주세요. (ex. link state, distance vector)](#id-29)
- [이더넷(ethernet)이 무엇인가요?](#id-30)
- [client와 server의 차이점을 설명해주세요.](#id-31)
- [delay, timing(jitter), throughput 차이를 설명해주세요.](#id-32)
...
네트워크 계층에서 보내온 데이터 정렬, 오류 정정 등을 수행하고 신뢰할 수 있는 통신을 확보한다. TCP/UDP 같은 프로토콜이 이 계층에 위치한다. TCP, UDP에 대한 내용은 [[#4] TCP와 UDP의 차이를 설명해주세요.](#id-4) 을 참고한다. 트랜스포트 계층 패킷을 **세그먼트**(segment)라고 한다.
...
OSI 7계층은 TCP/IP 계층의 애플리케이션 계층을 더 세분화한 것이다. TCP/IP 계층과 다른, 응용계층, 표현계층과 세션계층에 대해서 설명하고 나머지는 [[#1] CP/IP의 각 계층을 설명해주세요.](#id-1)을 참고한다.
...
소켓은 크게 UDP와 TCP 두 종류로 분류할 수 있다. 자세한 내용은 [#4. TCP와 UDP의 차이를 설명해주세요.](#id-4)을 참고!
...
- DHCP 서버는 보통은 라우터에 DHCP 서버가 내장되어 있으며 DHCP 서버에 연결된 클라이언트에게 자동적으로 IP주소를 할당한다. 자세한 내용은 [#28](#id-28)을 참고!
